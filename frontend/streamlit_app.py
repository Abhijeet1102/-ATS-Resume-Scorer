...
is_mock_mode = supabase_client._missing_config() is not None

for key, default in [
    ("access_token", "mock-token" if is_mock_mode else None),
    ("refresh_token", "mock-refresh-token" if is_mock_mode else None),
    ("user_id", "mock-user-id" if is_mock_mode else None),       # Supabase auth user id (uuid); also used by api_client
    ("user_email", "local-user@example.com" if is_mock_mode else None),
    ("auth_error", None),
    ("auth_info", None),
]:
...
with st.sidebar:
    ...
    if st.session_state.access_token:
        ...
        if is_mock_mode:
            st.info("⚡ Running in Local Mock Mode (No Supabase)")
        else:
